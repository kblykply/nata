    ...
    <section className="max-w-7xl mx-auto px-6 py-20">
      ...
      <div className="space-y-10">
        {posts.map((post) => (
          <div
            key={post.id}
            className="relative group flex flex-col md:flex-row justify-between items-center gap-6 bg-gray-50 p-6 rounded-xl transition"
          >
            {/* Hover Image */}
            ...
            <div className="md:w-1/3 flex justify-between items-start gap-4 z-20">
              ...
              <Link
                href={`/blog/${post.slug}`}
                className="border border-gray-300 text-xl rounded-full w-8 h-8 flex items-center justify-center hover:bg-white transition"
              >
                +
              </Link>
            </div>
